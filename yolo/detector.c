    ...
    float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers)
{
    ...
    while (1) {
        ...
        ulong elapsed = ((end_tv.tv_sec-start_tv.tv_sec)*1000000 + end_tv.tv_usec-start_tv.tv_usec);
        printf("Real time: %d us\r\n", elapsed);

        // MÓDOSÍTOTT
	double elapsed_millisecs = elapsed / 1000;
	fprintf(fptr,"%d\t%lf\n",frame_number,elapsed_millisecs);
        fflush(fptr);
